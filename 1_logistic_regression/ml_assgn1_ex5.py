...
new_features[:,1] = X[:,0]**2
new_features[:,2] = X[:,1]**2
new_features[:,1] = X[:,0]**3
new_features[:,2] = X[:,1]**3

column_dim = new_features.shape[1]
for i in range(column_dim):
   columns_new = np.expand_dims(new_features[:,i],axis = 1)
   X = np.append(X,columns_new,axis=1)
########################################/
